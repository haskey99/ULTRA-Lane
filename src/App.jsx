...
const CAR_BOTTOM = 14;
const HIT_ZONE_Y = 1 - CAR_BOTTOM / 100;
...
const CarSVG = memo(function CarSVG({ glow, lean = 0 }) {
...
const SIGN_W = 66, SIGN_H = 88;

const SpeedSign = memo(function SpeedSign({ speed, state, size = 1 }) {
  ...
  const rw = SIGN_W * size, rh = SIGN_H * size;
  return (
    <svg width={rw} height={rh} viewBox={`0 0 ${SIGN_W} ${SIGN_H}`} fill="none" shapeRendering="geometricPrecision">
      ...
      <text x="33" y="33" textAnchor="middle" dominantBaseline="middle" fontFamily="'Arial Black', Arial, sans-serif" fontWeight="900" fontSize={fs} fill={numCol} style={{ transition: "fill 0.15s" }}>{speed}</text>
    </svg>
  );
});

const Road = memo(function Road({ dashRef, blur }) {
  const dH = 5.5, dG = 7.5, tot = dH + dG;
  const dashes = [];
  for (let div = 1; div <= LANE_COUNT - 1; div++) {
    const x = div * LANE_W;
    for (let i = -1; i < 14; i++) {
      const y = (i * tot) % (100 + tot) - tot;
      dashes.push(<rect key={`${div}-${i}`} x={`${x - 0.38}%`} y={`${y}%`} width="0.76%" height={`${dH}%`} fill="rgba(255,210,64,0.85)" rx="1" />);
    }
  }
  return (
    <svg width="100%" height="100%" style={{ position: "absolute", inset: 0, pointerEvents: "none" }}>
      ...
      <g ref={dashRef}>{dashes}</g>
    </svg>
  );
});

const Tree = memo(function Tree({ x, y, s = 1, v = 0 }) {
...
const Lamppost = memo(function Lamppost({ x, y, s = 1 }) {
  const h = 36 * s;
...
const Bush = memo(function Bush({ x, y, s = 1 }) {
...
const SCENERY_SP = 120, SCENERY_N = 12;
const SCENERY_PERIOD = SCENERY_N * SCENERY_SP + SCENERY_SP;

const Scenery = memo(function Scenery({ scenRef, roadW }) {
  if (!roadW) return null;
  const items = [];
  for (let i = 0; i < SCENERY_N; i++) {
    const yL = ((i * SCENERY_SP) % SCENERY_PERIOD) - SCENERY_SP;
    const yR = ((i * SCENERY_SP + SCENERY_SP * 0.52) % SCENERY_PERIOD) - SCENERY_SP;
    const sc = 0.65 + (i % 3) * 0.15, v = i % 4, tL = i % 6, tR = (i + 2) % 6;
    if (tL === 1) items.push(<Lamppost key={`ll${i}`} x={-26} y={yL} s={sc * 0.85} />);
    else if (tL === 3) items.push(<Bush key={`lb${i}`} x={-14} y={yL} s={sc} />);
    else items.push(<Tree key={`lt${i}`} x={-18} y={yL} s={sc * 0.86} v={v} />);
    if (tR === 2) items.push(<Lamppost key={`rl${i}`} x={roadW + 26} y={yR} s={sc * 0.85} />);
    else if (tR === 4) items.push(<Bush key={`rb${i}`} x={roadW + 14} y={yR} s={sc} />);
    else items.push(<Tree key={`rt${i}`} x={roadW + 18} y={yR} s={sc * 0.86} v={(v + 2) % 4} />);
  }
  return (
    <svg width="100%" height="100%" style={{ position:"absolute", inset:0, pointerEvents:"none", overflow:"visible" }}>
      <rect x={-roadW} y={-30} width={roadW} height="130%" fill="#180f3d" />
      <rect x={roadW} y={-30} width={roadW} height="130%" fill="#180f3d" />
      <g ref={scenRef}>
        <g>{items}</g>
        <g transform={`translate(0, -${SCENERY_PERIOD})`}>{items}</g>
      </g>
    </svg>
  );
});
...
export default function UltraLane() {
  ...
  const dashOffValRef = useRef(0);
  const scenOffValRef = useRef(0);
  ...
  const startDrag = useCallback((clientX) => {
    if (phase !== "playing" || deadRef.current) return;
  ...
  const spawnSign = useCallback(() => {
    const isMatch = Math.random() < 0.30;
  ...
  const tick = useCallback((ts) => {
    ...
    dashOffValRef.current = (dashOffValRef.current + spd * dt * 100 * 1.5) % 13;
    if (dashRef.current) dashRef.current.style.transform = `translateY(${dashOffValRef.current}%)`;

    scenOffValRef.current = (scenOffValRef.current + spd * dt * 620) % SCENERY_PERIOD;
    if (scenRef.current) scenRef.current.style.transform = `translateY(${scenOffValRef.current}px)`;
  ...
  useEffect(() => { if (phase === "playing") { lastTRef.current = null; rafRef.current = requestAnimationFrame(tick); } return () => { cancelAnimationFrame(rafRef.current); lastTRef.current = null; }; }, [phase, tick]);
  ...
  useEffect(() => {
    if (phase === "playing") {
      ...
      signsRef.current = []; isDragRef.current = false;
      dashOffValRef.current = 0; scenOffValRef.current = 0;
      if (dashRef.current) dashRef.current.style.transform = "translateY(0%)";
      if (scenRef.current) scenRef.current.style.transform = "translateY(0px)";
      setScore(0); setStreak(0); setNewHS(false); setCarLane(1); setCarGlow(null); setCarLean(0); setCarPunch(false);
  ...
  if (phase === "intro" || phase === "result") {
    return (
      <div style={{ width:"100%", height:"100vh", background:PAGE_BG, display:"flex", flexDirection:"column", alignItems:"center", fontFamily:"'Trebuchet MS',Arial,sans-serif", overflow:"hidden", userSelect:"none", touchAction:"none", position:"relative" }}>
        <div style={{ position:"absolute", inset:0, opacity:0.22, pointerEvents:"none" }}><Road dashRef={dashRef} /></div>
        <div style={{ position:"relative", zIndex:10, display:"flex", alignItems:"center", justifyContent:"center", width:"100%", height:"100vh", paddingBottom:"18vh" }}>
          <div style={{ background:"rgba(26,17,60,0.92)", border:"1.5px solid rgba(255,111,97,0.4)", borderRadius:22, padding:"24px 22px 22px", maxWidth:340, width:"92%", display:"flex", flexDirection:"column", alignItems:"center", gap:14, boxShadow:"0 14px 60px rgba(20,8,50,0.7), 0 0 44px rgba(255,93,162,0.18)" }}>
            ...
            {phase === "intro" && (<><p style={{ color:"#b6a6e6", fontSize:12, margin:0, letterSpacing:1.5, textAlign:"center" }}>3 lanes. Drag your car. One chance.</p>{highScore > 0 && <div style={{ display:"flex", alignItems:"baseline", gap:8 }}><span style={{ color:"#9682cf", fontSize:11 }}>BEST</span><span style={{ color:"#ffd23f", fontSize:24, fontWeight:900, fontFamily:"'Arial Black',Arial,sans-serif" }}>{highScore}</span><span style={{ color:"#9682cf", fontSize:11 }}>CATCHES</span></div>}</>)}
  ...
  return (
    <div style={{ width:"100%", height:"100vh", background:PAGE_BG, display:"flex", flexDirection:"column", alignItems:"center", fontFamily:"'Trebuchet MS',Arial,sans-serif", overflow:"hidden", userSelect:"none", touchAction:"none", position:"relative" }}>
      <div style={{ width:"100%", maxWidth:440, display:"flex", justifyContent:"space-between", alignItems:"center", padding:"calc(9px + env(safe-area-inset-top, 0px)) 18px 8px", background:"linear-gradient(180deg, rgba(27,17,80,0.97), rgba(58,31,107,0.97))", borderBottom:"1px solid rgba(255,111,97,0.28)", zIndex:50, flexShrink:0 }}>
      ...
      <div ref={roadRef} style={{ flex:1, width:"100%", maxWidth:440, position:"relative", overflow:"hidden", cursor:"pointer" }}
        ...
        <Road dashRef={dashRef} blur={speedBlur} />
        <div style={{ position:"absolute", inset:0, pointerEvents:"none" }}><Scenery scenRef={scenRef} roadW={roadW} /></div>
        ...
        {signs.map(s => { const cx = s.lane * LANE_W + LANE_W / 2; const size = (0.45 + s.y * 0.9) * 0.8; const opa = s.state === "passed" ? Math.max(0, 1 - (s.y - HIT_ZONE_Y) * 8) : 1; return (<div key={s.id} style={{ position:"absolute", left:`${cx}%`, top:`${s.y * 100}%`, transform:"translate3d(-50%, -50%, 0)", willChange:"transform", pointerEvents:"none", zIndex:10, opacity:opa, transition:"opacity 0.1s", borderRadius:"50%", boxShadow: s.state === "correct" ? "0 0 14px #00e676" : s.state === "wrong" ? "0 0 14px #ff1744" : "0 2px 8px rgba(20,8,50,0.35)" }}><SpeedSign speed={s.speed} state={s.state} size={size} /></div>); })}
